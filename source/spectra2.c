      ...
      )
{
  ...
  if (psp->spectra_verbose > 0)
    printf (" -> computing second-order P(k)\n");
  ...
  for (int index_pk=0; index_pk<psp->pk_size; ++index_pk) {
    ...
    /* We compute the power spectrum in psp->k */
    double (*pk)[ppt2->tau_size];
    class_calloc (pk, psp->k_size*ppt2->tau_size, sizeof (double), psp->error_message);
    ...
    for (int index_k1 = 0; index_k1 < psp->k_size; ++index_k1) {
      ...
      for (int index_k2 = 0; index_k2 <= index_k1; ++index_k2) {
        ...
        //
        // class_alloc (
        //   interpolated_sources_in_k[index_pk],
        ...
        /* We now have a point in k1,k2 and interpolation in k3, we are ready to perform the integration */
        // for (int index_k3 = 0; index_k3 < psp->k_size; ++index_k3) {
        ...
        for (int index_k3 = 0; index_k3 < k3_size; ++index_k3) {
          ...
          /* Skip those configurations that do not satisfy the triangular condition.
          In principle this condition is already verified at the source level. */
          if (k1+k2<k3 || fabs(k1-k2)>k3)
            continue;
          ...
          // if (psp->k[index_k1] < psp->k[index_k3] /2.) {
          ...
          double triangular_step_k2 = step_k2;

          /* Loop over time, no integration */
          
          for (int index_tau = 0; index_tau < ppt2->tau_size; ++index_tau) {

            pk[index_k3][index_tau] +=
              ...
              // primordial spectra
               spectra_k1*spectra_k2*
              //sources
               sources (index_tau, index_k3) * sources (index_tau, index_k3)
               // interpolated_sources_in_k[index_tp][index_k3*ppt2->tau_size + index_tau]*
               // interpolated_sources_in_k[index_tp][index_k3*ppt2->tau_size + index_tau]
