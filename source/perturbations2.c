...
int perturb2_sources_at_k3 (
      ...
      double k3, /**< Input: k3 value where to interpolate the source function; ignored if index_k3 >=0 */
      ...
      )
{
  ...
  class_test (index_k1<0 || index_k1 >= ppt2->k_size,
    ppt2->error_message,
    "index_k1=%d out of bounds", index_k1);

  class_test (index_k2<0 || index_k2 >= ppt2->k_size,
    ppt2->error_message,
    "index_k2=%d out of bounds", index_k2);
#endif // DEBUG  
  ...
  /* Special case A: index_k2 is larger than index_k1. Accessing the source function
  in this case will result in segmentation fault, because SONG only computes S for
  k1>=k2. Here we swap the two indices and set a flag, so that before returning we
  can compute the correct result using the symmetry of the transfer functions. 
  This block of code must precede any attempt to access arrays involving index_k1
  and index_k2, lest incurring in segmentation faults. */
  
  int k1k2_was_swapped = _FALSE_;
  ...
  /* Special case B: index_k3 is a valid index. Then interpolation is not needed,
  and we return the source function at the tabulated value. */

  if (index_k3 >= 0) {

    class_test (index_k3 >= ppt2->k3_size[index_k1][index_k2],
      ppt2->error_message,
      "index_k3=%d is larger than k3_size[%d][%d]=%d",
      index_k3, index_k1, index_k2, ppt2->k3_size[index_k1][index_k2]);

    class_test (k3 != ppt2->k3[index_k1][index_k2][index_k3],
      ppt2->error_message,
      "inconsistent input");
  ...
  /* Special case C: if for this (k1,k2) there are no k3 nodes, then return zero
  ...
  /* Special case D: if for this (k1,k2) there is only one k3 node, return its value */
  ...
  /* Special case E: if the requested k3 is smaller than the first node, extrapolate
  ...
  /* Special case F: if the requested k3 is larger than the last node, extrapolate
  ...
  /* If the user initially gave index_k2 > index_k1, apply the symmetry factor
  (-1)^m and return */

  if (k1k2_was_swapped) {

    /* Extract azimuthal number of the required source, ie. find if it
    is a scalar, vector, tensor... */
    int index_m = ppt2->tp2_to_index_m[index_tp2];

    class_test (index_m < 0,
      ppt2->error_message,
      "could not extract m for index_tp2=%d, label=%s",
      index_tp2, ppt2->tp2_labels[index_tp2]);

    int m = ppt2->m[index_m];

    if (m != 0)
      *source *= ALTERNATING_SIGN(m);
        ...
        )
{
 ...
 C_l.");
  ...
  class_calloc (ppt2->file_header, 3*_MAX_INFO_SIZE_, sizeof(char), ppt2->error_message);

  /* Write information to appear on top of output files */
  sprintf (ppt2->file_header, "%s%sThis file was generated by SONG %s (%s) on %s.\n",
    ppt2->file_header, _COMMENT_, _SONG_VERSION_, _SONG_URL_, ppr->date);
  sprintf (ppt2->file_header, "%s%s\n", ppt2->file_header, _COMMENT_);
  sprintf (ppt2->file_header, "%s%s", ppt2->file_header, pba->info);
          ...
          )
{
  ...
  for (int index_tp = 0; index_tp < ppt2->tp2_size; index_tp++) {
    ...
    if (ppt2->has_source_T && index_tp >= ppt2->index_tp2_T && index_tp < ppt2->index_tp2_T+ppt2->n_sources_T) {
      ...
      offset2multipole_l_indexm (lm_offset, ppr2->l_max_los_t, ppt2->m, ppt2->m_size, &l, &index_m);
    ...
    /* - Photon E-mode polarization */
      ...
      int lm_offset = index_tp - ppt2->tp2_start[index_tp];
      offset2multipole_l_indexm (lm_offset, ppr2->l_max_los_p, ppt2->m, ppt2->m_size, &l, &index_m);
    ...
    /* - CDM density */
      ...
      sprintf(ppt2->tp2_labels[index_tp], "delta_cdm");
    ...
    /* - Magnetic field */
      ...
      sprintf(ppt2->tp2_labels[index_tp], "magnetic");
      
    }

    else {
      
      class_stop (ppt2->error_message, "could not identify index_tp2=%d", index_tp);
      
    }


    /* Check the result */
    class_test (l<0 || index_m<0 || index_m>=ppt2->m_size,
      ppt2->error_message,
      "index_tp2=%d: found (l,index_m)=(%d,%d) is out of bounds index_m=[%d,%d]\n",
      index_tp, l, index_m, 0, ppt2->m_size-1);    
          ...
          )
{
  ...
  for (int index_k1=0; index_k1 < ppt2->k_size; ++index_k1) {
    ...
    /* We shall compute the sources only for k2 smaller than k1, but here we compute
    the k-sampling arrays for all values of k2 */
    for (int index_k2=0; index_k2 < ppt2->k_size; ++index_k2) {
        ...
        )
{
  ...
  sprintf(line, "k1 = %g, k2 = %g, k3 = %g, cosk1k2 = %g, theta_1 = %g", 
          ...
          )
{
  ...
  void * (*pointer_tr) = calloc (_MAX_NUM_COLUMNS_, sizeof (void *));
  void * (*pointer_qs) = calloc (_MAX_NUM_COLUMNS_, sizeof (void *));
  ...
  if (has_k_out) {
    ...
    if (ppw2->n_steps == 1) {
      ...
      fprintf (file_tr, "%s%s\n", _COMMENT_, line);
      fprintf (file_tr, "%s", ppt2->file_header);
      fprintf (file_tr, "%s\n", _COMMENT_);
    ...
    if (ppt2->output_quadratic_sources == _TRUE_) {
      ...
      if (ppw2->n_steps==1) {
        ...
        fprintf (file_qs, "%s%s\n", _COMMENT_, line);
        fprintf (file_qs, "%s", ppt2->file_header);
        fprintf (file_tr, "%s\n", _COMMENT_);
        fprintf (file_tr, "%s", ppw2->info);
      }
        ...
        )
{
  ...
  for (int index_k_out=0; index_k_out < ppt2->k_out_size; ++index_k_out) {
    ...
    for (int index_tau_out=0; index_tau_out < ppt2->tau_out_size; ++index_tau_out) {
      ...
      fprintf (file_1D, "%sSecond-order source function S_lm(k1,k2,k3,tau) tabulated as a function of (k2,k3) for fixed (tau,k1).\n", _COMMENT_);
      fprintf (file_2D, "%sSecond-order source function S_lm(k1,k2,k3,tau) tabulated as a function of k3 for fixed (tau,k1,k2).\n", _COMMENT_);
      fprintf_2way (1, file_1D, 0, file_2D, 0, "%s", ppt2->file_header);
      fprintf_2way (1, file_1D, 0, file_2D, 0, "%s\n", _COMMENT_);
      fprintf_2way (1, file_1D, 0, file_2D, 0, "%sInformation on the output:\n", _COMMENT_);
      fprintf_2way (1, file_1D, 0, file_2D, 0, "%sk1 = %g, index_k1 = %d/%d\n", _COMMENT_, k1, index_k1, ppt2->k_size-1);
      fprintf (file_1D, "%sk2 = %g, index_k2 = %d/%d\n", _COMMENT_, k2, index_k2, ppt2->k_size-1);
      fprintf_2way (1, file_1D, 0, file_2D, 0, "%stau = %g, z = %g, a = %g, y = %g\n", _COMMENT_, tau, z, a, log10(a/pba->a_eq));


      /* For the 2D file, we shall print a row for each (k2,k3) configuration; for the
      1D file, a row for each k3 configuration */

      int n_rows_1D = 0;
      int n_rows_2D = 0;

      /* Note that we do a complete loop on k2, without restricting ourselves to k2<=k2 */
      ...
      for (int index_k2=0; index_k2 < ppt2->k_size; ++index_k2) {
        ...
        for (int index_k3=0; index_k3 < k3_size; ++index_k3) {
          ...
          source function only for k1>=k2. We obtain the values outside that range using
          the symmetry of the transfer functions with respect to permutations of (k1,k2). */
        ...
        )
{
  ...
  for (int index_k_out=0; index_k_out < ppt2->k_out_size; ++index_k_out) {
    ...
    sprintf (file->header, "%s%s", file->header, ppt2->file_header);
    file->header_size += strlen (ppt2->file_header) + 1;
    binary_sprintf (file, "");
