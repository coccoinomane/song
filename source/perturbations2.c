        ...
        )
{
  ...
  if (ppt2->has_matter_bispectrum == _TRUE_) {

    ppt2->has_source_delta_matter = _TRUE_;
    ppt2->index_tp2_delta_matter = index_type++;
          ...
          )
{
  ...
  for (int index_tp = 0; index_tp < ppt2->tp2_size; index_tp++) {
    ...
    && (index_tp >= ppt2->index_tp2_B) && (index_tp < ppt2->index_tp2_B+ppt2->n_sources_B)) {
    ...
    else if ((ppt2->has_source_delta_matter==_TRUE_) && (index_tp == ppt2->index_tp2_delta_matter)){
      ppt2->index_monopole[index_tp] = ppt2->index_tp2_delta_matter;
      l_max = 0;
      int lm_offset = index_tp - ppt2->index_monopole[index_tp];
      offset2multipole_l_indexm (lm_offset, l_max, ppt2->m, ppt2->m_size,
                                 &l, &index_m);
      ...
      /* Set the labels of the transfer types */
      sprintf(ppt2->tp2_labels[index_tp], "delta_m_%d_%d",l,ppt2->m[index_m]);
      /* Some debug */
      printf("delta_matter, index_tp=%d: lm_offset=%d -> (%d,%d), label=%s, monopole = %d\n",
      index_tp, lm_offset, l, ppt2->m[index_m],
      ppt2->tp2_labels[index_tp], ppt2->index_monopole[index_tp]);
    ...
    )
{
  ...
  if (ppt2->has_source_delta_matter == _TRUE_){
    double delta_cdm = 0.;
    double delta_cdm_1,delta_cdm_2, v_cdm_1, v_cdm_2, pressure_cdm;
    if (pba->has_cdm == _TRUE_) {
    ...
    /* Fill the ppt2->sources array */
    /*    sources(ppt2->index_tp2_delta_matter) =
          0.5*delta_cdm/(delta_cdm_1/(1.+3*Hc_sq/k1_sq)*delta_cdm_2/(1.+3*Hc_sq/k2_sq));*/
    sources(ppt2->index_tp2_delta_matter) =
      0.5*delta_cdm/delta_cdm_1/delta_cdm_2;
          ...
          )
{
  ...
  double kernel_delta_FSZ, delta_cdm_FSZ, alpha_FSZ, beta_FSZ, gamma_FSZ, Hc_over_k_sq;
  ...
  /* Eq. 60 from Fitzpatrick, Senatore and Zaldarriaga, 0902.2814. */
    Hc_over_k_sq = Hc_sq/k_sq;
    alpha_FSZ = 2./7. + 59./14.*Hc_over_k_sq + 45./2.*Hc_over_k_sq*Hc_over_k_sq;
    beta_FSZ = 1.0 - 0.5*Hc_over_k_sq + 54.0*Hc_over_k_sq*Hc_over_k_sq;
    gamma_FSZ = -1.5*Hc_over_k_sq + 4.5*Hc_over_k_sq*Hc_over_k_sq;
    kernel_delta_FSZ = beta_FSZ-alpha_FSZ + 0.5*beta_FSZ*cosk1k2*(k1/k2+k2/k1) +
      alpha_FSZ*cosk1k2*cosk1k2+gamma_FSZ*pow(k1/k2-k2/k1,2);
    delta_cdm_FSZ = 2*kernel_delta_FSZ*
      delta_cdm_1/(1.+3*Hc_sq/k1/k1)*
      delta_cdm_2/(1.+3*Hc_sq/k2/k2);
  ...
  if (pba->has_cdm == _TRUE_ ) {
    if (ppr2->compute_m[0] == _TRUE_) {
      ...
      // delta_cdm_analytical FSZ
      if (index_tau==0) fprintf(file_tr, format_label, "deltacdm_FSZ", index_print_tr++);
      else fprintf(file_tr, format_value, delta_cdm_FSZ);
      // v_0_cdm_analytical
