...
int bessel_get_l_list(
  ...
  /* Convert odd values to even ones. */
  
  if ((ppr->compute_only_even_ls==_TRUE_) || (ppr->compute_only_odd_ls==_TRUE_)) {

    int * l_copy;
    class_alloc (l_copy, pbs->l_size*sizeof(int), pbs->error_message);
    for (index_l=0; index_l < pbs->l_size; ++index_l)
      l_copy[index_l] = pbs->l[index_l];
    ...
    int index_l_copy = 0;
    index_l = 0;
    pbs->l[index_l] = l_copy[index_l_copy];
    index_l++;
    
    for (index_l_copy=1; index_l_copy < pbs->l_size; ++index_l_copy) {
  ...
  /* Find out the index in pbs->l corresponding to a given l. */
  class_alloc (pbs->index_l, (pbs->l[pbs->l_size-1]+1)*sizeof(int), pbs->error_message);

  int l;

  for(l=0; l<=pbs->l[pbs->l_size-1]; ++l) {
  
    pbs->index_l[l] = -1;
  
    for (index_l=0; index_l<pbs->l_size; ++index_l)
      if (l==pbs->l[index_l]) pbs->index_l[l] = index_l;
    
    /* Some debug */
    // printf("pbs->index_l[%d] = %d\n", l, pbs->index_l[l]);
  }
  ...
  if (pbs->bessels_verbose > 1) {
    printf ("     * ");
    for (index_l=0; index_l < (pbs->l_size-1); ++index_l)
