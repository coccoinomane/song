...
int transfer_init(
  ...
  if (ptr->transfer_verbose > 0)
    ...
    // else
    //   printf(" -> computing first-order transfer functions\n");
...
int transfer_indices_of_transfers(
  ...
  second order. In fact, at second-order, the k-list is determined within the module transfer2
  according to the k-sampling computed in tranfer2_get_k_list */
  ...
  for (index_mode = 0; index_mode < ptr->md_size; index_mode++) {
    ...
    /* We run this function only if we are not working at second order.  In fact, at
      second order the k-list is determined within the module transfer2 according to the
      k-sampling computed in transfer2_get_k_list */
...
int transfer_get_k_list(
  ...
  if ((ppt->has_scalars == _TRUE_) && (index_mode == ppt->index_md_scalars)) {
    ...
    /* RS_REC IS THE COMOVING SOUND HORIZON AT RECOMBINATION, tipically xxx */
    /* k_step_trans_scalars is around 0.004 */
    /* k_step_max = 6.28/xxx * 0.004 = */
  ...
  */
  ...
  for (index_k = 0; index_k < ptr->k_size[index_mode]; index_k++) {
	...
	if ((ppr->transfer_cut == tc_env) &&
       ...
       // *** MY MODIFICATIONS ***
	     ((ppt->has_cl_cmb_rayleigh == _TRUE_) && (index_tt == ptr->index_tt_r)) || 
       // *** END OF MY MODIFICATIONS ***
    ...
    /* above k_max: set transfer function to zero */
      ...
      // *** MY MODIFICATIONS ***
      // printf("l= %g, k= %g, max of ptr->k = %g\n", l, k, ptr->k[0][ptr->k_size[0]-1]);
      // *** END OF MY MODIFICATIONS ***
    }
...
/*   double bessel; */
  ...
  /* Print out the source function */
  // if ((l==2) && (index_k == 0)) {
  //   fprintf (stderr, "# l=%g, k=%g\n", l, k);
  //   fprintf (stderr, "%15s %15s %15s %15s\n", "tau", "source", "tau0_minus_tau", "delta_tau");
  // }
  ...
  for (index_tau=0; index_tau<index_tau_max; index_tau++) {
    ...
    /* Print out the source function */
    // if ((l==2) && (index_k == 0)) {
    //   double tau0 = 14142.863833;
    //   fprintf (stderr, "%15g %15g %15g %15g\n", tau0 - tau0_minus_tau[index_tau],
    //     sources[index_k * tau_size + index_tau], tau0_minus_tau[index_tau], delta_tau[index_tau]);
    // }
