...
int spectra_indices(
  ...
  if (ppt->has_cls == _TRUE_) {
    ...
    if ((ppt->has_cl_cmb_temperature == _TRUE_) && (ppt->has_cl_cmb_rayleigh == _TRUE_)) {
      psp->has_tr = _TRUE_;
    ...
    /* Cross correlation between polarization and the primordial curvature perturbation zeta */
...
#pragma omp flush(abort)
    ...
    smoother than the normal C_l's.  */
  	if (psp->compute_cl_derivative == _TRUE_) {
        ...
        for (index_ic2 = index_ic1; index_ic2 < psp->ic_size[index_mode]; index_ic2++) {
          ...
          for (index_l=0; index_l < ptr->l_size[index_mode]; index_l++) {
            ...
            for (index_ct=0; index_ct<psp->ct_size; index_ct++) {
              ...
              psp->lsq_cl[index_mode][(index_l * psp->ic_ic_size[index_mode] + index_ic1_ic2) * psp->ct_size + index_ct] =
                psp->cl[index_mode][(index_l * psp->ic_ic_size[index_mode] + index_ic1_ic2) * psp->ct_size + index_ct] * l * l;
      ...
      class_call(array_spline_table_lines(
                   psp->l,
  					       psp->l_size[index_mode],
  					       psp->lsq_cl[index_mode],
  					       psp->ic_ic_size[index_mode]*psp->ct_size,
  					       psp->dd_lsq_cl[index_mode],
                   _SPLINE_EST_DERIV_,
  					       psp->error_message),
        psp->error_message,
        psp->error_message);
...
int spectra_compute_cl(
  ...
  * 4. * _PI_ / k;
  ...
  primordial_pk[index_ic1_ic2]
  * transfer_ic1[ptr->index_tt_r]
  * transfer_ic2[ptr->index_tt_r]
  * 4. * _PI_ / k;
      
    if (psp->has_tr == _TRUE_)
      cl_integrand[index_k*cl_integrand_num_columns+1+psp->index_ct_tr]=
	primordial_pk[index_ic1_ic2]
	* 0.5*(transfer_ic1[ptr->index_tt_t] * transfer_ic2[ptr->index_tt_r] +
	       transfer_ic1[ptr->index_tt_r] * transfer_ic2[ptr->index_tt_t])
	* 4. * _PI_ / k;
  // *** END OF MY MODIFICATIONS ***
	...
	* 4. * _PI_ / k;
    ...
    if ((psp->has_tz == _TRUE_) && (ppt->has_scalars == _TRUE_) && (index_mode == ppt->index_md_scalars)) {
      cl_integrand[index_k*cl_integrand_num_columns+1+psp->index_ct_tz]=
  primordial_pk[index_ic1_ic2]
  * 0.5*(transfer_ic1[ptr->index_tt_t] * transfer_ic2[ptr->index_tt_zeta] +
         transfer_ic1[ptr->index_tt_zeta] * transfer_ic2[ptr->index_tt_t])
  * 4. * _PI_ / k;
    }

    /* Cross correlation between polarization and the primordial curvature perturbation zeta */
    if ((psp->has_ez == _TRUE_) && (ppt->has_scalars == _TRUE_) && (index_mode == ppt->index_md_scalars)) {
      cl_integrand[index_k*cl_integrand_num_columns+1+psp->index_ct_ez]=
  primordial_pk[index_ic1_ic2]
