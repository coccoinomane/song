...
int sp_lusolve(sp_num *N, double *b, double *x){
    ...
    /* NaN's sometimes arise here, because Ap contains infinites and NaN's */
...
/* VERSION IN THE NEW VERSION OF LASAGNA, BUGGY */
...
/* VERSION EMAILED TO ME BY THOMAS TRAM, WORKING? */
int column_grouping3(sp_mat *G, int *col_g, int *filled){
  int testcol,groupnum,fitted,neq;
  int i, *Ap, *Ai,done;
