...
int evolver_ndf15(
		  ...
		  double * t_vec, 
		  int tres,             		  /* tres is the size of t_vec */
  ...
  // *********************   START OF MAIN LOOP *********************
  if (verbose > 3) printf("Entering main loop\n");
  ...
  while (done==_FALSE_){
	...
	*/
	...
	for(iter=1;iter<=maxit;iter++){
      ...
      /* NaN's sometimes arise here, see comment in the function implementation in
        the sparse.c module */
	...
	if (tooslow==_TRUE_){
      ...
      if (exit_strategy != NULL) {
        ...
        class_call ((*exit_strategy)(
                  		  ...
                  		  t_vec, 
                  		  tres,             		  /* tres is the size of t_vec */
                  		  output,
                  		  print_variables,
                        error_message),
          error_message,
    ...
    if (verbose>3) printf("\tEntering the output part of the main loop\n");


    /** Output **/
    // RICORDA CHE:
    // 1) siamo alla fine di una iterazione del main loop 'while (done==_FALSE_)'. Le due quantità più
    //    importanti del ciclo sono tnew (valore della variabile indipendente dopo l'utlimo step),
    //    e t_vec[next] (valore di t_vec più vicino a tnew da sinistra).
    //
    // 2) al primo ciclo del main loop, 'next' è la posizione dove t_vec (i.e. tau_sampling) diventa maggiore di t0
    //    (inizio dell'intervallo che stiamo considerando).  Se non c'è overlap tra t_vec e l'intervallo preso in
    //    considerazione, allora next = 0.
    //
    // 3) tdir è una flag che indica la direzione. E' sempre = 1 tranne quando tnew > tfinal, nel qual caso è = -1.
    //
    // Il ciclo sottostante farà tante iterazioni quanti sono gli elementi di 't_vec' tra t_vec[next] e tnew
    // (i.e. quelli introdotti nell'ultimo step dell'evolver).  Quel che conta sono gli elementi di t_vec.
    // Se lo step tra i due elementi di t_vec è più grande dello step dell'evolver, la condizione è falsa
    // in partenza e il codice nemmeno entra nel ciclo.  Però, alla successiva iterazione del main loop,
    // tnew sarà più grande mentre next sarà uguale, e quindi c'è la possibilità che tnew abbia superato
    // t_vec[next].
    //
    // La filosofia è: non me ne pò fregà de meno di tutti gli step che non siano quelli contenuti in t_vec.
    // Quindi li butto via.
    // A noi però questi step interessano perchè le quantità che stiamo evolvendo sono le sources delle nostre 
    // equazioni al second'ordine e quindi ci servono anche a tempi molto precedenti a quelli contenuti in
    // t_vec (che va dalla ricombinazione ad oggi).
    //
    // Vedo due possibili approcci:
    // 1) ci teniamo tutti gli steps che scarta CLASS, cambiando di poco questo loop o aggiungendone
    //    uno parallelo controllato da una flag, oppure
    // 2) dentro perturb_solve, mettiamo una chiamata opzionale a generic_evolver in cui specifichiamo
    //    noi l'intervallo temporale che ci interessa.
    while ((next<tres)&&(tdir * (tnew - t_vec[next]) >= 0.0)){
      
      /* Do we need to write output? */
      // SE HAI PRESO ESATTAMENTE IL PUNTO CHE VOLEVI, SALVA NELL'OUTPUT.
      // Il codice entra in questo 'if' solo una volta per k-mode, quando siamo arrivati a tau_fin
      if (tnew==t_vec[next]){
        class_call((*output)(t_vec[next],ynew+1,f0+1,next,parameters_and_workspace_for_derivs,error_message),
          ...
          // *** MY MODIFICATIONS
          // We need the time index (next) in order to print the labels.
          class_call((*print_variables)(t_vec[next],ynew+1,f0+1,next,
      ...
      else {  // SE NON HAI PRESO ESATTAMENTE IL PUNTO CHE VOLEVI, INTERPOLA.
        ...
        class_call((*output)(t_vec[next],yinterp+1,ypinterp+1,next,parameters_and_workspace_for_derivs,
          ...
          // *** MY MODIFICATIONS
          // We need the time index (next) in order to print the labels, and we want the interpolated
          // values in order to have a smooth result          
          class_call((*print_variables)(t_vec[next],yinterp+1,ypinterp+1,next,
                         parameters_and_workspace_for_derivs,error_message),
                        error_message,error_message);
      ...
      next++;
    }  //  END OF: while ((next<tres)&&(tdir * (tnew - t_vec[next]) >= 0.0))
    /** End of output **/
    if (done==_TRUE_) {
      break;
    }
  ...
  // *********************   OUT OF MAIN LOOP *********************
  if (verbose > 3) printf("Ouf of main loop\n");
...
int new_linearisation(struct jacobian *jac,double hinvGak,int neq,ErrorMsg error_message){
  ...
  if(jac->use_sparse==1){
    ...
    /* Change suggested by Thomas Tram to solve the nan bug */
  ...
  */
  ...
  if ((jac->use_sparse)&&(jac->repeated_pattern >= jac->trust_sparse)){
    ...
    if (jac->has_grouping==0){
      ...
      // *** MY MODIFICATIONS ***
      /* Use the version of column_grouping that Thomas Tram sent me via email */
      jac->max_group = column_grouping3(jac->spJ,jac->col_group,jac->col_wi);
      // *** END OF MY MODIFICATIONS ***
...
int initialize_jacobian(struct jacobian *jac, int neq, ErrorMsg error_message){
  ...
  // *** MY MODIFICATIONS ***
  /* We disable sparse matrices because every now and then they give rise
    they produce NaN's (see comments in sp_lusolve in sparse.c) */
  // jac->use_sparse = 0;
